...
# --- FIX: Changed from a relative to an absolute import ---
...
def generate_predictions(config):
    ...
    for stock in stocks:
        ...
        output_data = {'stock': stock, 'historical': stock_df[['date', 'open', 'high', 'low', 'close']].to_dict(orient='records'), 'predictions': prediction_df.assign(date=lambda df: df.date.dt.strftime('%Y-%m-%d')).to_dict(orient='records'), 'summary': summary, 'accuracy': {'mape': mape, 'directional_accuracy': directional_accuracy}}
