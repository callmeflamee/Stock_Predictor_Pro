...
def process_data(config):
    ...
    if 'tweet_sentiment' in merged_df.columns:
        merged_df['tweet_sentiment'].fillna(0, inplace=True)
    else:
        merged_df['tweet_sentiment'] = 0
    if 'news_sentiment' in merged_df.columns:
        merged_df['news_sentiment'].fillna(0, inplace=True)
    else:
        merged_df['news_sentiment'] = 0

    weights_tweet = (merged_df['tweet_sentiment'] != 0).astype(int)
    weights_news = (merged_df['news_sentiment'] != 0).astype(int)
    total_weight = weights_tweet + weights_news
    total_weight[total_weight == 0] = 1
    merged_df['sentiment'] = (merged_df['tweet_sentiment'] * weights_tweet + merged_df['news_sentiment'] * weights_news) / total_weight
    
    merged_df.drop(columns=['tweet_sentiment', 'news_sentiment'], inplace=True, errors='ignore')

    # --- CRITICAL FIX: Ensure data is sorted chronologically before saving ---
    merged_df.sort_values(by=['stock', 'date'], inplace=True)
    
    os.makedirs(os.path.dirname(processed_data_path), exist_ok=True)
    merged_df.to_csv(processed_data_path, index=False)
    print(f"Processed data with new, intelligent sentiment blend saved to {processed_data_path}")
